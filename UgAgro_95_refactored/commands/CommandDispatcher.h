...
public:
  ...
  void registerCommand(ICommand* cmd) {
    if (cmd) {
      ...
      if (debugMode) {
        Serial.printf("CommandDispatcher: Registered command: %s\n", cmd->getName());
      }
    }
  }
  ...
  bool dispatch(const char* name, JsonDocument& doc, bool& needSave) {
    ...
    if (it != commands.end()) {
      if (debugMode) {
        Serial.printf("CommandDispatcher: Executing command: %s\n", name);
      }
      if (it->second->validate(doc)) {
        return it->second->execute(doc, needSave);
      } else {
        if (debugMode) {
          Serial.printf("CommandDispatcher: Validation failed for command: %s\n", name);
        }
        return false;
      }
    }
